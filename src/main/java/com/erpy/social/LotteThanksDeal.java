 ...
 */
public class LotteThanksDeal {
    private static Logger logger = Logger.getLogger(LotteThanksDeal.class.getName());
    ...
    //
    private static final String prefixContentUrl = "http://m.lotteimall.com";
    ...
    public Map<String, SearchData> extract(CrawlData crawlData) throws Exception {
        ...
        for (Element element : elements) {
            ...
            if (document.select("p.stitle").size() == 0)
                continue;

            // Thumb link
            listE = document.select("div.area img");
            ...
            for (Element et : listE) {
                ...
                if (strLinkUrl.length()>0) {
                    productId = globalUtils.getFieldData(strLinkUrl, "goods_no=", "&");
            ...
            // shipping
            listE = document.select("li.impending span");
            for (Element et : listE) {
            ...
            // product name
            listE = document.select("p.stitle");
            for (Element et : listE) {
...
//            }
            ...
            // sell count
            listE = document.select("span.buy em");
            for (Element et : listE) {
                strItem = globalUtils.priceDataCleaner(et.text());
                if (strItem.length()>0 && GlobalUtils.isAllDigitChar(strItem)) {
            ...
            // sale per
            listE = document.select("span.sale");
            for (Element et : listE) {
                strItem = globalUtils.priceDataCleaner(et.text());
                if (strItem.length()>0 && GlobalUtils.isAllDigitChar(strItem)) {
    ...
    public void mainExtractProcessing(LotteThanksDeal cp,
                                      CrawlData crawlData,
    ...
    public static void main(String args[]) throws Exception {
        ...
        crawlSite.setCrawlUrl("http://m.lotteimall.com/main/getMainTab.lotte?tabNo=1&glog=14103_1");
...
//            }
            ...
            // end time
            listE = document.select("span.time");
            for (Element et : listE) {
                strItem = et.attr("data-end");
                if (GlobalUtils.isAllDigitChar(strItem)) {
                    logger.info(String.format(" >> end time(%s)", strItem));
                    break;
                }
            }
