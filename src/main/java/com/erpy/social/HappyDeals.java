...
/**
 * Created by baeonejune on 15. 4. 26..
 * NS HOME Shopping.
 */
public class HappyDeals {
    private static Logger logger = Logger.getLogger(HappyDeals.class.getName());
    ...
    //
    private static final String prefixContentUrl = "http://m.nsmall.com";
    ...
    public Map<String, SearchData> extract(CrawlData crawlData) throws Exception {
        ...
        for (Element element : elements) {
            if (!element.outerHtml().contains("<dd class=\"info infoDead\">"))
                continue;
            productId="";
            ...
            // Thumb link
            listE = document.select("dl img");
            for (Element et : listE) {
                strItem = et.attr("tsrc");
            ...
            for (Element et : listE) {
                ...
                if (strLinkUrl.length()>0) {
                    productId = globalUtils.getFieldData(strLinkUrl, "good_id=");
...
//            }
            ...
            // product name
            listE = document.select("a p.brandtitle");
            for (Element et : listE) {
            ...
            // org price
            listE = document.select("span.price del");
            for (Element et : listE) {
                ...
                if (strItem.length()>0 && GlobalUtils.isAllDigitChar(strItem)) {
                    ...
                    logger.warn(String.format(" Extract [org price] data is NOT valid - (%s)", strItem));
                    logger.warn(String.format(" Extract [org price] product name      - (%s)", searchData.getProductName()));
                    logger.warn(String.format(" Extract [org price] seed url          - (%s)", crawlData.getSeedUrl()));
                    logger.warn(" -------------------------------------------------------------------------");
                }
            }

            // sale price
            listE = document.select("span.price p");
            for (Element et : listE) {
                if (et.outerHtml().contains("<del>")) continue;
                strItem = globalUtils.priceDataCleaner(et.text());
                if (strItem.length()>0 && GlobalUtils.isAllDigitChar(strItem)) {
                    ...
                    logger.warn(String.format(" Extract [sale price] data is NOT valid - (%s)", strItem));
                    logger.warn(String.format(" Extract [sale price] product name      - (%s)", searchData.getProductName()));
                    logger.warn(String.format(" Extract [sale price] seed url          - (%s)", crawlData.getSeedUrl()));
                    logger.warn(" -------------------------------------------------------------------------");
                }
            }

            // sell count
            listE = document.select("span.deadline p");
            for (Element et : listE) {
                if (!et.outerHtml().contains("판매중")) continue;
                Elements items = et.select("strong");
                for (Element ele : items) {
                    strItem = globalUtils.priceDataCleaner(ele.text());
                    if (strItem.length() > 0 && GlobalUtils.isAllDigitChar(strItem)) {
                        searchData.setSellCount(Integer.parseInt(strItem));
                        logger.debug(String.format(" >> sell count(%s)", searchData.getSellCount()));
                        break;
                    } else {
                        logger.warn(String.format(" Extract [sell count] data is NOT valid - (%s)", strItem));
                        logger.warn(String.format(" Extract [sell count] product name      - (%s)", searchData.getProductName()));
                        logger.warn(String.format(" Extract [sell count] crawl url         - (%s)", crawlData.getSeedUrl()));
                        logger.warn(" -------------------------------------------------------------------------");
                    }
                }
            }
...
//            }
            ...
            // set cp name.
            searchData.setCpName(GlobalInfo.CP_HappyDeals);
            // set keyword.
    ...
    public void mainExtractProcessing(HappyDeals cp,
                                      CrawlData crawlData,
...
//            }
    ...
    public static void main(String args[]) throws Exception {
        ...
        crawlSite.setCrawlUrl("http://m.nsmall.com/mjsp/index.jsp?tab_id=02");
        crawlSite.HttpCrawlGetDataTimeout();
        happyDeals.extractTest(crawlSite.getCrawlData());
    }
}
