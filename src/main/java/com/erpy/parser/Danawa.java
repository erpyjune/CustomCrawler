 ...
 */
public class Danawa {
    private static Logger logger = Logger.getLogger(Danawa.class.getName());
    // for extract.
...
//        physicsCate4:0
//        viewMethod:LIST
//        sortMethod:CMRecommend
//        listCount:30
//        group:13
//        depth:3
//        listPackageType:0
//        categoryMappingCode:422
//        priceUnit:0
//        priceUnitValue:0
//        cmRecommendSort:Y
//        cmRecommendSortDefault:Y
//        bundleImagePreview:N

        heaer.put("Host","prod.danawa.com");
        heaer.put("Origin","http://prod.danawa.com");
        heaer.put("Pragma","no-cache");
        ...
        heaer.put("Referer", "http://prod.danawa.com");
        heaer.put("Connection", "keep-alive");
        heaer.put("Content-Type", "application/x-www-form-urlencoded");
        heaer.put("Cookie", "userAccessIP=61.106.107.68; userAccessTime=2015-08-23+20%3A19%3A50; cookPrevViewCategory=133460%7C132437; ACEFCID=UID-55727FA425535CA3591C055B; ACEUCI=1; OAX=PWprIFVyf5EADliG; _AT_vid=BJ3JOYPJR4NFLBQ72D5DTC8Z7X280701; RMFL=011Z16J5U10jH; danawa-loggingApplicationClient=a9e2a6cd-9b9b-496b-932c-0270f4d24c4d; cookSaveShopInfo=TH201%3A2015-07-25; RMFD=011ZTTIyO1010x; cookieGuestId=fc0e99a1c717ce374553737fdb35fd8c; _gat=1; _EXEN=3; _ga=GA1.2.832942935.1433567140");
        crawl.setRequestHeader(heaer);

        crawl.setCrawlUrl("http://prod.danawa.com/list/ajax/getProductList.ajax.php");
        crawl.setCrawlEncode("utf-8");

        param.put("page", "2");
        param.put("btnAllOptUse", "true");
        param.put("listCategoryCode", "460");
        param.put("categoryCode", "460");
        param.put("physicsCate1", "1825");
        param.put("physicsCate2", "32097");
        param.put("physicsCate3", "51242");
        param.put("physicsCate4", "0");
        param.put("viewMethod", "LIST");
        param.put("sortMethod", "CMRecommend");
        param.put("listCount", "30");
        param.put("group", "13");
        param.put("depth", "3");
        param.put("listPackageType", "0");
        param.put("categoryMappingCode", "422");
        param.put("priceUnit", "0");
        param.put("priceUnitValue", "0");
        param.put("cmRecommendSort", "Y");
        param.put("cmRecommendSortDefault", "Y");
        param.put("bundleImagePreview", "N");
...
//        logger.info(crawl.getCrawlData());
        ...
        Elements elements = doc.select("li.prod_item");
        for (Element element : elements) {
...
//            logger.info(" =======================================");
            ...
            // thumb
            Elements elist = document.select("div.thumb_image a img");
            for (Element eitem : elist) {
                logger.info(" thumb : http:" + eitem.attr("src"));
            }

            // link
            Elements elink = document.select("div.thumb_image a");
            for (Element eitem : elink) {
                String strItem = eitem.attr("href");
                String brandcode = globalUtils.getFieldData(strItem, "pcode=", "&");
                String pid = globalUtils.getFieldData(strItem, "pid=");
                logger.info(" url : " + strItem);
            }

            // product name
            Elements listE = document.select("div.head_info strong");
            for (Element et : listE) {
            ...
            // spec1
            Elements ePrice = document.select("div.spec_list a");
            for (Element eitem : ePrice) {
                String strItem = eitem.text();
                logger.info(String.format(" spec1 : %s", strItem));
            }

            // spec2
            Elements eSpec = document.select("div.spec_list");
            for (Element eitem : eSpec) {
                String strItem = eitem.text();
                logger.info(String.format(" spec2 : %s", strItem));
            }

            // sale price
            Elements eSalePrice = document.select("dl.low_price span.num.num1");
            for (Element eitem : eSalePrice) {
                logger.info(String.format(" sale_price : %s", globalUtils.priceDataCleaner(eitem.text())));
            }
