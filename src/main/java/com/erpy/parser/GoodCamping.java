...
/**
 * Created by baeonejune on 15. 6. 15..
 */
public class GoodCamping {
    private static Logger logger = Logger.getLogger(GoodCamping.class.getName());
    ...
    //
    private static final String prefixContentUrl = "http://goodcamping.net";
    private static final String prefixHostThumbUrl = "http://goodcamping.net";
    ...
    public Map<String, SearchData> extract(CrawlData crawlData) throws Exception {
        ...
        // 파싱 시작.
        elements = doc.select("td[style=\"padding-bottom:25px;\"]");
        for (Element element : elements) {

            if (!element.outerHtml().contains("../goods/goods_view.php"))
                continue;
            ...
            // Thumb link
            listE = document.select("div[style=\"text-align:center;\"] img");
            ...
            for (Element et : listE) {
                ...
                if (strLinkUrl.length()>0) {
                    ...
                    searchData.setContentUrl(prefixContentUrl + strLinkUrl.replace("../good","/shop/good"));
            ...
            // product name
            listE = document.select("div[style=\"padding:14px 0 0 6px; text-align:left; width:130px;\"] div a");
            for (Element et : listE) {
            ...
            // org price
            listE = document.select("div[style=\"padding:14px 0 0 6px; text-align:left; width:130px;\"] a strike[style=\"font-size:11px;\"]");
            for (Element et : listE) {
            ...
            // sale price
            listE = document.select("div[style=\"padding:14px 0 0 6px; text-align:left; width:130px;\"] div[style=\"padding-bottom:3px;\"] b");
            for (Element et : listE) {
            ...
            // set cp name.
            searchData.setCpName(GlobalInfo.CP_GoodCamping);
            // set keyword.
    ...
    public void mainExtractProcessing(GoodCamping cp,
                                      CrawlData crawlData,
    ...
    public static void main(String args[]) throws Exception {
        ...
        crawlSite.setCrawlUrl("http://goodcamping.net/shop/goods/goods_list.php?category=001001");
...
//        logger.info(String.format(" crawl contents size : %d", crawlSite.getCrawlData().length()));
        ...
        for (Element element : elements) {
            ...
            for (Element et : listE) {
                ...
                if (strLinkUrl.length()>0) {
                    productId = globalUtils.getFieldData(strLinkUrl, "goodsno=","&");
                    logger.info(" url : " + prefixContentUrl + strLinkUrl.replace("../","/shop/"));
                    break;
                }
            }
